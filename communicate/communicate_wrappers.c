...
/*========================================================================*/
void Sendrecv_replace(void *buf, int size, MPI_Datatype DATATYPE, int sent_to,
                      int tag_send, int recv_from, int tag_recv, MPI_Comm COMM)

{
#ifdef PARALLEL
   MPI_Status status;
   MPI_Sendrecv_replace(buf,size,DATATYPE,sent_to,tag_send,recv_from,tag_recv,
                         COMM,&status);

#else
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
      printf("No Sendrecv_replace in scalar\n");
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
      fflush(stdout);
      exit(1);
#endif
