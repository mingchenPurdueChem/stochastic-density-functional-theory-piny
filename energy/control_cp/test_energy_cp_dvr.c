...
void test_energy_cp_dvr(CLASS *class, BONDED *bonded, 
...
#include "../typ_defs/typ_mask.h"
  ...
  double dvrfc_up_b,dvrfc_dn_b;
/* Local constants */
  int ninter         = 17;
  double delta       = 1.e-5;
  double delta_cp    = 1.e-7;
...
/* Local pointers */
  ...
  int ncoef_r          = cp->cp_para_fft_pkg3d_sm.nfft/2;
  int ncoef_r_proc     = cp->cp_para_fft_pkg3d_sm.nfft_proc/2;
  ...
  /*
  int icoef_up_st      = cp->cpcoeffs_info_dvr.icoef_start_up;
  int icoef_dn_st      = cp->cpcoeffs_info_dvr.icoef_start_dn;
  */
  ...
  double *cpcoeffs_dvrc_up  = cp->cpcoeffs_pos_dvr[1].dvrc_up;
  double *cpcoeffs_dvrc_dn  = cp->cpcoeffs_pos_dvr[1].dvrc_dn;
  double *cpcoeffs_dvrfc_up = cp->cpcoeffs_pos_dvr[1].dvrfc_up;
  double *cpcoeffs_dvrfc_dn = cp->cpcoeffs_pos_dvr[1].dvrfc_dn;
  ...
  /*
  int nstate_ncoef_proc_max_up = cp->cpcoeffs_info_dvr.nstate_ncoef_proc_max_up;
  int nstate_ncoef_proc_max_dn = cp->cpcoeffs_info_dvr.nstate_ncoef_proc_max_dn; 
  int nstate_ncoef_proc_up     = cp->cpcoeffs_info_dvr.nstate_ncoef_proc_up;
  int nstate_ncoef_proc_dn     = cp->cpcoeffs_info_dvr.nstate_ncoef_proc_dn;
  */
  ...
  icoef_up_end = icoef_up_st + nstate_ncoef_proc_up - 1;
  icoef_dn_end = icoef_dn_st + nstate_ncoef_proc_up - 1;

  if(np_states==0){
    nstate_ncoef_proc_max_up  = ncoef_r;
    nstate_ncoef_proc_max_dn  = ncoef_r;
    nstate_ncoef_proc_up      = ncoef_r;
    nstate_ncoef_proc_dn      = ncoef_r;
    icoef_up_st  = 1;
    icoef_dn_st  = 1;
    icoef_up_end = ncoef_r;
    icoef_dn_end = ncoef_r;
  }/*endif*/
...
/* II) Malloc CP states */
  ...
  /* CHECK MALLOCS */
  nmall_up = ncoef_r_proc*nstate_up;
  dvrfc_up = (double *)cmalloc(nmall_up*sizeof(double))-1;

  if(cp_lsda == 1 && nstate_dn_proc != 0){
   nmall_dn = ncoef_r_proc*nstate_dn; 
   dvrfc_dn = (double *)cmalloc(nmall_dn*sizeof(double))-1;
  }/*endif*/
...
/* A) Get the true force */
  ...
  if(inter!=3){
    ...
    get_ghost_pos(&(class->clatoms_info),&(class->clatoms_pos[1]),
                  &(class->ghost_atoms));
    if(num_proc>1){Barrier(comm_states);}
    test_force_dvr_cp(class,bonded,general_data,cp,inter,&vgen,&vgen2,&vgen3); 
    if(num_proc>1){Barrier(comm_states);}
    cputime(&cpu2); 
    cpu = cpu2-cpu1;
  }/*endif*/
...
/* B) Save the force, pressure tensor,cp force                               */
    ...
    if(inter==17){
      cp->cpcoeffs_pos_dvr[1].ifcoef_form_up = 1; 
      for(i=1;i <= nmall_up;i++){
        dvrfc_up[i] = cpcoeffs_dvrfc_up[i];
      }
      if(cp_lsda== 1 && nstate_dn != 0){
        cp->cpcoeffs_pos[1].ifcoef_form_dn = 1;
        for(i=1;i <= nmall_dn;i++){
          dvrfc_dn[i] = cpcoeffs_dvrfc_dn[i];
        }
      }/* endif cp_lsda */
    }/* endif inter */
...
/*  i) X forces */
        ...
        get_ghost_pos(&(class->clatoms_info),&(class->clatoms_pos[1]),
                      &(class->ghost_atoms));
        test_force_dvr_cp(class,bonded,general_data,cp,inter,
                          &vgen_p,&vgen2,&vgen3); 
      ...
      if(ifp_check == 1) {
        ...
        if(myid_state==0){
         ...
         scanf("%d %d",&icoef_orig,&istate);printf("\n");
          if(icoef_orig > ncoef_r || icoef_orig <= 0) {
        ...
        ind = (istate-1)*nstate_ncoef_proc_up + icoef;

        if(num_proc>1){Barrier(comm_states);}
        if(myid_state==0 && icoef > 0){
          dvrfc_up_now = dvrfc_up[ind];
   	}/*endif*/
        if(myid_state==0 && icoef == 0){        
          Recv(&dvrfc_up_now,1,MPI_DOUBLE,MPI_ANY_SOURCE,1,comm_states);
	}/*endif*/
        if(myid_state!=0 && icoef != 0){        
          Send(&dvrfc_up[ind],1,MPI_DOUBLE,0,1,comm_states);
	}/*endif*/

/* Doing real part of up states */
        if(icoef>0){cpcoeffs_dvrc_up[ind] += delta_cp;}
        test_force_dvr_cp(class,bonded,general_data,cp,inter,
                                                &vgen_p,&vgen2,&vgen3); 

        if(icoef>0){cpcoeffs_dvrc_up[ind] -= 2.0*delta_cp;}
        test_force_dvr_cp(class,bonded,general_data,cp,inter,
                                                &vgen_m,&vgen2,&vgen3); 

        if(icoef>0){cpcoeffs_dvrc_up[ind] += delta_cp;}
        if(myid_state==0){dvrfc_up_b = (vgen_m-vgen_p)/(2.0*delta_cp);}


/*  i) Print out */
        if(num_proc>1){Barrier(comm_states);}
        if(myid_state==0){
       printf("coef,state %d %d dvrfc_up_num %.13lg dvrfc_up %.13lg \n %.13lg %.13lg\n",
          icoef_orig,istate,dvrfc_up_b,dvrfc_up_now,
          fabs(dvrfc_up_b-dvrfc_up_now),dvrfc_up_b/dvrfc_up_now);
         }/*endif*/
        if(num_proc>1){Barrier(comm_states);}
...
/* Doing real part of dn states */
        if(cp_lsda == 1 && nstate_dn != 0){
          ...
          ind = (istate-1)*nstate_ncoef_proc_dn + icoef;

          if(myid_state==0 && icoef == 0){        
            Recv(&dvrfc_dn_now,1,MPI_DOUBLE,MPI_ANY_SOURCE,1,comm_states);
	  }/*endif*/
          if(myid_state!=0 && icoef != 0){        
            Send(&dvrfc_dn[ind],1,MPI_DOUBLE,0,1,comm_states);
   	  }/*endif*/

          if(icoef>0){cpcoeffs_dvrfc_dn[ind] += delta_cp;}
          test_force_dvr_cp(class,bonded,general_data,cp,inter,
                                                &vgen_p,&vgen2,&vgen3); 

          if(icoef>0){cpcoeffs_dvrfc_dn[ind] -= 2.0*delta_cp;}
          test_force_dvr_cp(class,bonded,general_data,cp,inter,
                                                &vgen_m,&vgen2,&vgen3); 

          if(icoef>0){cpcoeffs_dvrfc_dn[ind] += delta_cp;}
          if(myid_state==0){dvrfc_dn_b = (vgen_m-vgen_p)/(2.0*delta_cp);}

/*  i) Print out */
          if(num_proc>1){Barrier(comm_states);}
          if(myid_state>0){
            printf("coef,state %d %d dvrfc_dn_num %.13g dvrfc_dn %.13g\n",
                   icoef_orig,istate,dvrfc_dn_b,dvrfc_dn_now);
          }/*endif*/
          if(num_proc>1){Barrier(comm_states);}
        }/* endif cp_lsda */
...
void test_force_dvr_cp(CLASS *class, BONDED *bonded, 
...
#define DEBUG_CP
      ...
      *vgen = *vgen2 + (general_data->stat_avg.vrecip); 
      ...
      if(myid_state==0){
        printf("reciprocal energy: %.12g\n",general_data->stat_avg.vrecip);
        printf("Total energy: %.12g\n",*vgen);
      }
