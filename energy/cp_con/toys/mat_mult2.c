...
/* I) Initialization */
  ...
  nrecv_b   = nm*nl_proc_b_now;
  for(im=1;im<=nsend_a;im++){b_old[im]=b[im];}
...
/* II) Calculation loop */
  ...
  for(il=nl_proc_a_st;il<=nl_proc_a_end;il++){
   joff=0;
   for(jl=nl_proc_b_st_now;jl<=nl_proc_b_end_now;jl++){
    knd = jl + (il-1)*nl_tot;
    for(im=1;im<=nm;im++){
     ind = im + ioff;
     jnd = im + joff;
  ...
  for(iproc=0;iproc<num_proc;iproc++){
   if(myid==iproc) {
       printf("myid = %d nl_a_st %d nl_a_end %d nl_b_st %d nl_b_end %d\n",
        myid,nl_proc_a_st,nl_proc_a_end,nl_proc_b_st_now,nl_proc_b_end_now);
   }/*endif*/
   scanf("%d",&il);
   MPI_Barrier(MPI_COMM_WORLD);
  }/*endfor*/
...
/* V) Collect, symmetrize and reduce matrix */
 ...
 for(il=1;il<=nl_tot2;il++){c_red[il] = c[il];}
 ...
 MPI_Barrier(MPI_COMM_WORLD);
 if(myid==0){
  for(il=1;il<=nl_tot2;il++){printf("c[%d] = %.12g\n",il,c[il]);}
 }/*endif*/
#endif
