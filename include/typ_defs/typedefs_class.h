...
/*                                                                          */
typedef struct ggmt {
  double G[3], Q[3], vt[3], qt[3], w[4], dT;
  int nr, ny;
  double tau, kTs;
} GGMT;

typedef struct dev_tri{
   double dev_list[4][3][2];
}DEV_TRI;

typedef struct dafed_cv {
 int type;          // type of CV
 int bdy;           // boundary condtion
 int *i1, *i2;      // utility use
 int num_atm_list;  // number of atoms in this CV 
 int *atm_list;     // atom indeies
 int i1_num, i2_num;// utility use
 double min,max;    // min(max) boundary of the CV
 double diff;       // max-min
 double k_bdy;      // the force constant if we choose harmonic bdry
 double kTs;        // temperature for extended variable 
 double ms;         // mass for extended variable  
 double ks;         // harmonic coulping
 double  q;         // CV value, calculate from the configuration
 double  s;         // extend variable value
 double  vs;        // extend variable velocity
 double  Fs;        // force on extended variable
 double dt;         // time step
 int bin_num;       // numerical bin numbers for biasing potential
 int lat_num;       // lattice point number on this dimension, =bin_num+1
 double w_bin;      // numerical bin size for biasing potential
 //double *force;  
 double kin,kin_ave;     // kinetic energy 
 double pot_harm;        // harmonic coulpling energy
 //double pot_bias;      // biasing potential
 double therm_k,therm_k_ave; // thermostat kinetic energy
 double therm_p;         // thermostat potential energy
 double temp,temp_ave;   // temperature calculate from 
 double *Fx,*Fy,*Fz;     // force on atoms
 GGMT therm;             // thermostat variables
 DEV_TRI *dev;           // use for dihedral angles type of CV only
 //int n_atm_f;
 //int *atm_fl;
} DAFED;

typedef struct bias_hist{
  double *x;
  double gaussian_v;
}BIAS_HIST;

typedef struct c_nblt{
  int *diff;
  double sign;
}C_NBLT;

typedef struct dafed_info {
 int dafed_on;
 int n_cv;
 //int *natm_cv;
 //int **atm_map_dafed;
 ...
 int i_bias;
 int ggmt_nr;
 int nres_daf;          //respa for dafed
 double fac_bias;
 double ggmt_tau;
 double timestep;
 char *traj_fn;
 int bias_readin;       //bias potential readin on=1,off=0
 int calc_flag;         //so we dont use respa for bias force calculation
 double sigma;          //gaussian sigma
 double A;              //gaussian A
 int bias_write_freq;   //writing frequency of biased informantion
 int write_freq;
 int print_screen_freq;
 /*  for biasing potential  */
 //double *gau_v;    
 int *n_lat;            //number of lattice points for each level of chunck
 int n_lat_tot;         //total number of lattic points
 int nblt_tot;          //total number of neibourhood latice points
 double pot_bias;       //biasing potential
 double ene_tot;        //total energy(therm+kinetic+harmonic+biasing)

 C_NBLT *nblt_array;    //neibourhood lattice points
 BIAS_HIST *bhist;
} DAFED_INFO;
