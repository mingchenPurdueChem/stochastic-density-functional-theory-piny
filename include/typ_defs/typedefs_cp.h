...
typedef struct cp_comm_state_pkg {
   ...
   int nstate_proc_min;        /* defined for DVR */
...
typedef struct cpopts{
  ...
  int cp_init_min_opt;
  int iwrite_init_wcent;
  int iwrite_init_worb;
  int iwrite_init_state;
  ...
  int metalFlag;                  /* Opt: If we are calculating metallic system    */
  int smearingType;               /* Opt: smearingfunction type 1=Fermi            */
  double elecT;                   /* Num: electron temperature                     */
  double numElecTrueUp,numElecTrueDn;           
                                  /* Num: number of true electrons                 */
} CPOPTS;
...
typedef struct cpcoeffs_info {
  ...
  int cp_elf_calc_frq;         /* Num: Compute the electron localization
                                       function evey ... steps         */
  int cp_dip_calc_frq;         /* Num: Compute the dipole moment*/

  int cp_laplacian_on;         /* Num: Compute the Laplacian of the density */

  int iopt_cp_pw;              /* Opt: Plane wave basis set */

  int iopt_cp_dvr;             /* Opt: DVR basis set */
  ...
  int grid_nx,grid_ny,grid_nz;      /* Num: grid # for each dimension */
  double scale_fact;                /* Volume divided by number of grid */
  double dvr_grid_dens;         /* Num: dvr grid density */
...
/* I want to store some variables here so that I can pass them everytime */
  ...
  int *numElecDnFragTot;	    /* Lst: Number of spin down electrons in    */
				    /*	    one fragment			*/
				    /* Lth: numFragTot				*/
  int *numElecTrueUpFragTot;        /* Lst: Number of True spin up electrons    */
                                    /*      in the fragment (metallic system)   */ 
  int *numElecTrueDnFragTot;        /* Lst: Number of True spin down electrons  */
                                    /*      in the fragment (metallic system)   */
  int *numElecTrueUpFragProc;       /* Lst: Number of spin up electrons in one  */
                                    /*      fragment (proc version,metallic)    */
                                    /* Lth: numFragProc                         */
  int *numElecTrueDnFragProc;       /* Lst: Number of spin down electrons in    */
                                    /*      one fragment (proc version,metallic)*/
                                    /* Lth: numFragProc                         */
...
// Try overlap box
  ...
  int *gridShift;		    /* Lst: If skin=0.5UC, this tells the diff  */
				    ...
				    /*	    FFT grid.				*/
				    /* Lth: numFragProc*3			*/
  FILE *fragScreenOut;              /* FILE: fragment screen output             */
  int fragDFTMethod;                /* Opt: Method you want to use for fragment */
                                    /*      DFT calculation 1=cg, 2=fd          */
  double fragOrbRatio;              /* Num: If you use fd, the number of states */
                                    /*      generated by fd=fragOrbRatio*       */
                                    /*      N_occ where N_occ=N_e/2             */
  int fragNumStateStoUp;            /* Num: If fragDFTMethod=2, the number of   */
  int fragNumStateStoDn;            /*      stochastic orbitals.                */
  int fragNumChemPot;               /* Num: If fragDFTMethod=2, the number of   */
                                    /*      chemical potentials                 */
  double fragBeta;                  /* Num: If fragDFTMethod=2, the beta value  */
}FRAGINFO;
...
/* This structure contains flag and constant. */
  ...
  int iScfTrue;                     /* Num: SCF loop index,used for reading     */
                                    /*      checkpoint file                     */
...
typedef struct cpcoeffs_pos_dvr {
  ...
  double *dvrc_up;                 /* Lst: DVR coeff spin up
                                      Lth: ncoef x nstate_up             */
  double *dvrc_dn;                 /* Lst: DVR coeff spin dn
                                      Lth: (0,ncoef)*nstate_dn           */
  double *dvrvc_up;                /* Lst: DVR coeff velocities spin up
                                      Lth: ncoef x nstate_up             */
  double *dvrvc_dn;                 /* Lst: DVR coeff velocities spin dn
                                      Lth: (0,ncoef)*nstate_dn           */
  double *dvrfc_up;                /* Lst: DVR coeff forces spin up
                                      Lth: ncoef x nstate_up             */
  double *dvrfc_dn;                /* Lst: DVR coeff forces spin dn
                                      Lth: (0,ncoef)*nstate_dn           */

  /*
  double *fcre_up,*fcim_up;        
                                   
  double *fcre_dn,*fcim_dn;        
  */
                                   

  double *fpc_up, *fpc_dn;         /* preconditioned force */
...
typedef struct dvr_matrix{

  double *Tsx,*Tsy,*Tsz;            /* Small T matrix for kinetic energy
                                       Lth: grid_nx(y,z) x grid_nx(y,z)     */
  double *dGx,*dGy,*dGz;           /* Lst: derivative of grid basis functions */
                                   /*  wrt x,y,z coordinates Lth: ngrid_x(y,z)^2*/
  double *TRx, *TIx,*TRy,*TIy,*TRz,*TIz; /* Real and imaginary parts of
                                        DVR-FBR transformation matrix */
  double *Tfbr_x, *Tfbr_y, *Tfbr_z; /* diagonal kinetic energy matrix
                                       in FBR   */
...
/*==========================================================================*/
/*               Cluster option for DVR                                    */


typedef struct cp_dvr_clus{
...
typedef struct electronic_properties{
  double *cp_elf_up,*cp_elf_dn; /* Lst: Becke-Edgecomb electronic localization
                                        function
                                        Lth: nfft2                          */
  double **wannier_cent;      /* x,y,z components of Wannier center        */
  double *weight;
  double ***Z_real,***Z_imag;
  double *elementA;
  double **cp_dip;

  double *HMat_1,*HMat_2,*HMat_3,*GMat_1,*GMat_2,*GMat_3; /* for dipole moment */

} ELECTRONIC_PROPERTIES;

/*==========================================================================*/
/*                  WANNIER related properties                              */
/*                                                                          */

typedef struct cp_wannier{
  int cp_wan_calc_frq;   /* Num: Compute the wan functions every xx steps*/
  int wan_func_typ;      /* Num: 1,2, or 3; Resta,Silvestrelli,Berghold */
  int wan_diag_typ;      /* Num: 1,2, or 3: LQ,divide and conquer, RRR */
  int nmax_wan_orb;      /* Num: maximum number of wannier orbitals used for
                                each atom in nonlocal energy calculation */
  double rcut_wan_orb, rcut_wan_nl;
} CP_WANNIER;
...
typedef struct pseudo{
  ...
  double b3_cut;               /* Num: short-range exact exchange cut-off  */
  double b3_alp;               /* Num: short-range exact exchange alpha */
  ...
  int nsplin_r_tot;            /* Num: Total # of r-space spline pts  */
  int *nsplin_r;               /* Lst: Number of real space spline points for DVR */
                               /* in pseudopotential file DVR Lth (natm_typ) */
  double *dr_spl;              /* Num: Spacing between r-vectors pts */
                               /*   Lth (natm_typ) */
  double *rmin;                /* Lst: Minimum radial distance in pseudo file */
                               /*   = rmax/nsplin_r Lth (natm_typ) */
  /*
  double *rmax;                
  double *z_1,*z_2;            
  double *alp_1,*alp_2;       
  */
  ...
  int *nl_filter;   /* real-space filtering option for each atom type */
  ...
  double *nl_alp, *nl_beta;   /* real-space fourier filtering parameters
                                 Lth: (natm_typ) */ 

  double *phi0_0, *phi0_1, *phi0_2;
  ...
  /* Exact Exchange related variables... */
  double erfcg_corr;

  /* For future use
  int ngrid_b3_comm, nproc_b3_comm,ngrid_sphere,nblock_max_comm;
  int **b3_comm_map,**b3_comm_num_fine;
  int *b3_send_order, *b3_recv_order;
  double *erfc_map;            
  double *erfc_fft;
  int *kap_sphere,*kbp_sphere,*kcp_sphere,*kindex_all;
...
typedef struct cpscr_nonloc{
 /* All done on the small dense grid  which is dens_cp_box */
  ...
  double *vnlre_dn,*vnlim_dn;  /* Lst: List of nl pseudopot 
                                  ...
                                  Lth: nlscr_dn_proc                       */
  double *dvnlre_up,*dvnlim_up;  /* Lst: List derivative of Zilmn
                                 wrt real space coefficient used for DVR;
                                 Lth: nlscr_up as defined for DVR          */
  double *dvnlre_dn,*dvnlim_dn;  /* Lst: List derivative of Zilmn
                                 wrt real space coefficient used for DVR;
                                 Lth: nlscr_dn_proc as defined for DVR     */
  ...
  double *vnlre_tmp, *vnlim_tmp;
  double *dvnlre_x_tmp, *dvnlim_x_tmp;
  double *dvnlre_y_tmp, *dvnlim_y_tmp;
  double *dvnlre_z_tmp, *dvnlim_z_tmp;
  ...
  /* Nonlocal spherical trunction, or Nonlocal Wannier truncation stuff */
  int nloc_trunc_on, nloc_wan_on, nmax_wan_orb, nrmax_nl;
  int *my_wan_orb, *num_wan_orb, *cnum_wan_orb,*igrid_map;
  double *dvrc_wan;
...
typedef struct cpscr_grho{
 /* All done on the small dense grid */
  ...
  double *d_rhox_up_c,*d_rhoy_up_c,*d_rhoz_up_c;
                 /* Lst: partial del_rho(x,y,z)/partial cijk */
                 /* Lth: nfft2 */
...
typedef struct cpscr_wave{
  ...
  int *ifft,*jfft,*kfft;      /* Map Arrays used to rerrange DVR data for FFT */
...
typedef struct cpscr_wannier{

  int cp_wannier_on;

  double *U_final, *g, *gg, *diag; /* Temp array for newton and lbfg_min */

  double **A, **R_real, **R_imag, **U_real, **U_imag;
  double **U_tmp1, **U_tmp2,**U_tmp3, **U_tmp4, **Bt_real, **Bt_imag, **M_real;

  double *real, *imag, *D, *norm;
  double **phi, **HMatrix;
