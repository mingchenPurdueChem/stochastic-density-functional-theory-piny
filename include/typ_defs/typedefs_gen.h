...
typedef struct minopts {
  ...
  double tol_wan_coef;         /* Num: Tol on wannier minimization    */
...
typedef struct filenames {
  ...
  char *wcoutname;             /* Chr: Wannier Center output file name*/
  ...
  int iwrite_elf; 
  int iwrite_wan;              /* Num: Freq. of writes for wannier coefficients */
  int iwrite_wan_cent;         /* Num: Freq. of writes for wannier centers */
  int iwrite_dipole;
} FILENAMES;
...
/* 2.*/
  ...
  int *mapFFTWFilter,*mapConFFTWFilter;
  double cputime;
