...
void cp_shuffle_states_dvr(CP *cp,int ip)
...
/* I) UP STATES */
  ...
  if( nstate_up > 1 ){
    cp_shuffle_prim_dvr(dvrc_up,icoef_form_up,cpscr_cre_up,ioff_upt,
                        occ_up,rocc_sum_up,occ_dn,cp_lsda,
                        &(cp->vel_samp_cp),&(cp->cp_comm_state_pkg_dvr_up));
  }/*endif*/
...
/* II) DN STATES */
  ...
  if( nstate_dn > 1 && cp_lsda == 1){
    cp_shuffle_prim_dvr(dvrc_dn,icoef_form_dn,cpscr_cre_dn,ioff_dnt,
                        occ_dn,rocc_sum_dn,occ_up,cp_lsda,
                        &(cp->vel_samp_cp),&(cp->cp_comm_state_pkg_dvr_dn));
  }/*endif*/
...
void cp_shuffle_prim_dvr(double *cpcoefs_cre,int icoef_form,double *cpscr_cre,int *iofft,
                         double *occ,double *rocc_sum,double *occ_dn, int cp_lsda,
                         VEL_SAMP_CP *vel_samp_cp,CP_COMM_STATE_PKG *cp_comm_state_pkg)
