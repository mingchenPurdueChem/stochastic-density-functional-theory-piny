...
void int_dt2_to_dt_nvt(CLASS *class,BONDED *bonded,GENERAL_DATA *general_data,
                       int ir_tra,int ir_tor,int ir_ter,int ir_daf,double dt)
...
/* II) Evolve NHCs and velocities                                           */
   ...
   if( (int_res_tra==0) && (int_res_ter==0) ){
	...
	&& class->therm_info_class.len_nhc == 3) {
     ...
     ix_now = 5;
     if((ir_daf==nres_daf)){ix_now=4;}
     if((ir_daf==nres_daf)&&(ir_tra==nres_tra)){ix_now=3;}
     if((ir_daf==nres_daf)&&(ir_tra==nres_tra)&&(ir_tor==nres_tor))
						    {ix_now=2;}
     if((ir_daf==nres_daf)&&(ir_tra==nres_tra)&&(ir_tor==nres_tor)&&(ir_ter==nres_ter))
                                                    {ix_now=1;}
     if(ix_respa>=ix_now){
      if(class->therm_info_class.therm_typ == 1) {
       apply_NHC_par(&(class->clatoms_info),&(class->clatoms_pos[1]),
