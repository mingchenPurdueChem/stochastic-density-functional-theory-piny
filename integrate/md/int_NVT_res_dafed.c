...
/*cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*/
/*==========================================================================*/
void int_NVT_res_dafed(CLASS *class,BONDED *bonded,GENERAL_DATA *general_data)
/*========================================================================*/
{/*begin routine*/
/*========================================================================*/
/*             Local variable declarations                                */
    ...
    int ir_tra,ir_tor,ir_ter,ir_daf;
    int ix_now;
    int nres_tra,nres_tor,nres_ter,nres_daf;
    int int_res_ter,iii;
...
/* 0) Useful constants                                                      */
    ...
    nres_daf = dinfo->nres_daf;
    dt   = (general_data->timeinfo.dt);
    dti  = dt/((double)(nres_ter*nres_tor*nres_tra*nres_daf));
    dti2 = dti/2.0;
...
/*==========================================================================*/
/*==========================================================================*/
/* IV) Loop over dafed RESPA                                               */
 
           for (ir_daf=1; ir_daf<=nres_daf; ir_daf++) {
...
/* 1) Evolve system from t=0 to dt/2                                        */
    if(ir_ter==1&&ir_tor==1&&ir_tra==1&&ir_daf==1)int_ggmt_flag = 1;
    else int_ggmt_flag = 0;
   
    int_0_to_dt2_nvt(class,bonded,general_data,ir_tra,ir_tor,ir_ter,ir_daf,dti);
    int_dafed_0_to_dt2_cl(class,general_data,int_ggmt_flag,dti);
...
/* 2) Get the new energy/force                                              */
    ...
    else energy_control_only_dafed(class,bonded,general_data);

/*==========================================================================*/
/* 3) Evolve system from dt/2 to dt                                         */

        int_dt2_to_dt_nvt(class,bonded,general_data,ir_tra,ir_tor,ir_ter,ir_daf,dti);
    if(ir_ter==nres_ter&&ir_tor==nres_tor&&ir_tra==nres_tra&&ir_daf==nres_daf)int_ggmt_flag = 1;
    else int_ggmt_flag = 0;
     
        int_dafed_dt2_to_dt_cl(class,general_data,int_ggmt_flag,dti);
