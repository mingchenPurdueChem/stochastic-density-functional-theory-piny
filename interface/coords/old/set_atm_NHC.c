...
void set_atm_NHC(ENSOPTS *ensopts, STATEPOINT *statepoint, 
...
/*   1) couple all molecules of this type to global nhc                     */
    if(imol_nhc_opt[jmol_typ]==1){
      if(ind_glob_nhc == 0){
        class_num_nhc+= 1;
        ind_glob_nhc = class_num_nhc;
        class_mass_nhc[1][ind_glob_nhc] = (tau_nhc_def)*(tau_nhc_def);
        class_gkt[1][ind_glob_nhc] = 0.0;
        class_text_nhc[(class_num_nhc)]  = state_t_ext;
      }/*endif*/
...
/*   2) couple all molecules of this type to the same nhc                  */
    if(imol_nhc_opt[jmol_typ]==2){
      ...
      class_mass_nhc[1][(class_num_nhc)] = tau_nhc_mol[jmol_typ]*
                                           tau_nhc_mol[jmol_typ];
      class_text_nhc[(class_num_nhc)]    = text_nhc_mol[jmol_typ];
      for(jatm=istart;jatm<=iend;jatm++){
  ...
  if(therm_info_class->therm_typ == 1){
   for(j=2;j<=class_len_nhc;j++){
    for(i=1;i<=class_num_nhc;i++){
      ...
      class_mass_nhc[j][i] = class_mass_nhc[1][i]*class_text_nhc[i]/BOLTZ;
    }/*endfor*/
   }/*endfor*/
...
/* Get GGMT thermostat masses*/
    ...
    for(i=1;i<=class_num_nhc;i++){ 
     ...
     class_mass_nhc[2][i] = class_mass_nhc[1][i]*class_gkt[1][i]*text
     ...
     if(class_len_nhc == 3){
       ...
       /*debug*/
       printf("cst3=%.12g,cst4=%.12g,class_mass[3][%d]=%.12g\n",
       cst3,cst4,i,class_mass_nhc[3][i]);
     }/*endif*/
    }/*endfor*/
  }/*endelse*/
/*==========================================================================*/
/* First thermostat masses */
   ...
   for(i=1;i<=class_num_nhc;i++){
    class_mass_nhc[1][i] = class_mass_nhc[1][i]*class_gkt[1][i]
                          *class_text_nhc[i]/BOLTZ;
    ...
    for(jatm=1;jatm<=natm_tot;jatm++){      
      ...
      if(ighost_flag[jatm] == 0) {
        if(pimd_freez_typ==1 || freeze_flag[jatm]==0){
           ...
           bead_text_nhc[(bead_num_nhc)]     = text_nhc_mol[jmol_typ];
