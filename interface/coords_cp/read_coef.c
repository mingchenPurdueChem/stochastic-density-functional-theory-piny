...
/*  V) Allocate and initialize coefficient arrays                         */
  ...
  if(iopt_cp_dvr){
    read_coef_alloc_init_dvr(cp,cp_min_on,tot_memory);
    if(num_proc>1){Barrier(world);}
  }/*endif*/
...
/* VIII) Read/Communicate Coeffs Vels                                     */
  ...
  if((istart >= 3)&&((cp_wave_min+cp_min)==0)) {
    ...
    if(iopt_cp_dvr==1){
      read_coef_fetch_vcoefs_dvr(cp,fp_dnameci,dnameci);
    }
  } /* endif for istart >=3 */
  if(num_proc>1){Barrier(world);}
...
/* IX) Read/communicate Extended system variables                         */
  ...
  if((istart == 4)&&((cp_wave_min+cp_min)==0)) {
    ...
    if(iopt_cp_dvr==1){
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
      printf(" Coefficients thermostat is comming soon! \n");
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
...
/*==========================================================================*/
  ...
  if(iopt_cp_dvr==1){
    if( ((istart <= 2)||((cp_wave_min+cp_min)==1)) && (istart != 0) ){
      control_coef_transpose_fwd(cp,1);
      for(ip=1;ip<=pi_beads_proc;ip++){
        cp->cpcoeffs_pos_dvr[ip].ivcoef_form_up = 1;
        cp->cpcoeffs_pos_dvr[ip].ivcoef_form_dn = 1;
      }
    }
    /* no need to transpose for gen_wave */
    if(istart == 0){
      for(ip=1;ip<=pi_beads_proc;ip++){
        cp->cpcoeffs_pos_dvr[ip].icoef_form_up = 1;
        cp->cpcoeffs_pos_dvr[ip].icoef_form_dn = 1;
        cp->cpcoeffs_pos_dvr[ip].ivcoef_form_up = 1;
        cp->cpcoeffs_pos_dvr[ip].ivcoef_form_dn = 1;
      }
    }
...
void read_wan_cent(CP *cp,GENERAL_DATA *general_data)
...
/* Local pointers */
  ...
  char *wname          = general_data->filenames.wcinname;
  double *WCF;

  if(myid==0){
    fp_wname = cfopen(wname,"r");

    printf("\n");PRINT_LINE_STAR;
    printf("Reading user specified Wannier center file %s\n\n",wname);

    for (i=1;i<=nstate;i++){
      WCF=cp->electronic_properties.wannier_cent[i];
      fscanf(fp_wname,"%lf %lf %lf %d",&(WCF[1]),&(WCF[2]),&(WCF[3]),&dummy);
    }
    fclose(fp_wname);
  }
  ...
  if(num_proc > 1){
    Barrier(comm);
    Bcast(&(cp->electronic_properties.wannier_cent[1][1]),3*nstate,MPI_DOUBLE,0,comm);
  }
