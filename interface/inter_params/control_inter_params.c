...
/*=======================================================================*/
/*  6) Convert and assign Grimme                                        */
  
  if(strcasecmp(dict[3].keyarg,"grimme") == 0) {
    inter_base[ibase].inter_label = 7;
    sscanf(dict[9].keyarg,"%lg",&c6m);
    sscanf(dict[17].keyarg,"%lg",&RvdW);
    sscanf(dict[18].keyarg,"%lg",&s6);
    if(RvdW<0){
      index=17;
      keyarg_barf(dict,filename,fun_key,index);
    }/*endif*/
    ...
    inter_base[ibase].c6m        = c6m*s6/(BOLTZ*bohr6);
    inter_base[ibase].rm_swit    = RvdW/BOHR;
  }/*endif*/
...
/*             Local variable declarations                                */
  ...
  int nunique_mall;
  int nlj,nnull,n3,n5,n6,n7,ntot,nother;
  int i,j,k,iii;
...
/*----------------------------------------------*/
/* 2. inter_label=6 Aziz-Chen second to last */
/*----------------------------------------------*/
...
/*---------------------------------------------*/
/* 3. inter_label=5 Williams-LJ third to last */
/*---------------------------------------------*/
...
/*------------------------------------------*/
/* 4. inter_label= 3 Williams fourth to last */
/*------------------------------------------*/
...
/*--------------------------------------------*/
/* 5. PLACE inter_label=4 null fifth to last */
/*--------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
/* Assign Grimme compare c6m rm_swit */
/* and cutoffs  */
/*--------------------------------------------------------------------------*/

   jlow = nunique - nother + n3 + n5 + n6 + 1; 
   jup  = jlow + n7;
