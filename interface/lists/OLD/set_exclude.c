...
/*       classical atoms which were excluded                            */
   ...
   if(error_check_on==1){
      PRINT_LINE_STAR;
      printf("Determining CP-CP ewald corrections \n");
      PRINT_LINE_DASH;printf("\n");
   }/*endif*/


 /*Count number of pairs ab initio - ab initio */
 /* This will occur if you define a bond between 2 ab iniiot atoms*/
 /* which were included in exclusion list, but whose coloumbic   */
...
/* AB INITIO - CLASSICAL and AB INITIO - AB INITIO */
/*       &&((clatoms_info->cp_atm_flag[(jtmp1[i])] == 1)
          ||(clatoms_info->cp_atm_flag[(jtmp2[i])] == 1))){  */
	bonded->excl.num_cp++;
      }/*endif*/
