...
/*   VI) Assign Flags                                                     */
  ...
  cp->cpcoeffs_info.ecutFrag = cp_parse.cp_ecut_def;
...
/* Set up CP and Ewald stuff                                                */
    ...
    if(iopt_cp_dvr ==1){
      control_set_cp_ewald_dvr(&(general_data->simopts),&(general_data->cell),
                               &(cp->cpcoeffs_info),&(general_data->ewald),
                               &(cp->cpewald),&cp_parse,
                               &(cp->pseudo.gmin_spl),
                               &(cp->pseudo.gmax_spl),
                               class_parse.kmax_ewd, tot_memory,
                               general_data->timeinfo.int_res_ter,
                               &(bonded->ecor),myid,cp_dual_grid_opt_on,
                               class->part_mesh.pme_on);
    }
...
/*  Calculate Number of CP fictitious degrees of freedom                    */
    ...
    if(cp_on) {
      ...
      if(iopt_cp_pw==1){ calculate_cp_nfree(cp); }
      if(iopt_cp_dvr==1){ calculate_cp_nfree_dvr(cp); }
    ...
    if(myid_state < num_proc && myid_state >= 0){
      ...
      if(iopt_cp_dvr == 1) {
        control_fft_pkg_dvr(&(cp->cp_sclr_fft_pkg3d_sm),
                            ...
                            &(cp->cp_para_fft_pkg3d_lg),
                            &(general_data->ewald),&(cp->cpewald),
                            &(cp->cpcoeffs_info), &(class->communicate),
                            cp_on, tot_memory, cp->cpopts.cp_para_opt,
                            cp_dual_grid_opt_on, &(cp->cpscr));
      }
    }/* endif */
  }/*endif*/
  ...
  if(iopt_cp_dvr == 1){
    if(iperd == 0){
      if(general_data->simopts.cp_wave_min == 1) {
         move_to_cell_center(class);
      }
      get_dvr_cp_clus_fmat(cp,&(general_data->cell),tot_memory);
    }
  }
...
/* must be done after control_mall_scratch is called  */
    ...
    /* Setup DVR kinetic energy matrices Tsx and derivative of basis functions*/

    if(iopt_cp_dvr == 1){
      gen_Ts_dvr(&(cp->dvr_matrix), &(cp->cpcoeffs_info),
                 &(general_data->cell),iperd,myid);
      gen_dgrid_dvr(&(cp->dvr_matrix),&(cp->cpcoeffs_info),
                    &(general_data->cell),iperd,myid);
      gen_dvr_fbr(&(cp->dvr_matrix),&(cp->cpcoeffs_info),
                  &(general_data->cell), &(cp->cpscr.cpscr_wave),
                  iperd,myid,cp_parse.cp_mass_cut_def);
    }
...
/* XXIII) Orthogonalize coefficients                                      */
  ...
  if(myid_state<np_states){
    control_init_cp_orthog(general_data,cp,&cp_parse,cp_on,cp_md,myid);
  }/*endif*/
...
/*                (interface/vel_sampl)                                   */
  ...
  if(cp_md==1){
    if(myid_state<np_states){
     control_init_coef_vsmpl(general_data,cp,&cp_parse,myid);
    }/*endif*/
  }/*endif*/
...
/*                (dafed/)                                                */
  ...
  if(num_proc>1)Bcast(&(dinfo->n_cv),1,MPI_INT,0,world);
  int n_cv = dinfo->n_cv;
...
void control_init_cp_orthog(GENERAL_DATA *general_data,CP *cp,
                            CP_PARSE *cp_parse,int cp_on,int cp_md, int myid)
    ...
    for(ip=1;ip<=cp->cpcoeffs_info.pi_beads_proc;ip++){
      ...
      if(iopt_cp_dvr==1){
        ...
        if((cp_parse->istart_cp > 2)&&(cp_parse->ivc_smpl == 0)&&(cp_md==1)){
          if(myid==0){
            printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@\n");
            printf("             IMPLEMENT PROJ_VELC_DVR \n");
            printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@\n");
          }/*endif*/
          fflush(stdout);
          exit(1);
...
/* I) Coeficients                                                         */
      ...
      if((cp_parse->istart_cp <= 2) || (cp_parse->ivc_smpl == 1)){
        ...
        if(iopt_cp_dvr){
          ...
          control_vc_smpl_dvr(general_data,cp); 
        }
      }/*endif*/
      ...
      if(cp_parse->ivc_scale == 1){
        ...
        if(iopt_cp_dvr){
          if(myid==0){
            printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@\n");
            printf("     IMPLEMENT CONTROL_VC_SCALE_DVR \n");
            printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@\n");
          }/*endif*/
          fflush(stdout);
          exit(1); 
