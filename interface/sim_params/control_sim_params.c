...
/*  III) Malloc some memory */
  ...
  general_data->filenames.wcoutname  = (char *)cmalloc(MAXWORD*sizeof(char));
...
/* IV) Open the data file and read in the information                    */
  ...
  nline = 1;
  nfun_key=0;
  flag_test=get_fun_key(fp,fun_key,&nline,&nfun_key,input_name);
...
/*   V) Write out the simulation parameters to the simulation file        */
  ...
  write_simfile(fp,dict_dafed,num_dict_dafed,dict_fun[25].keyword);
  fclose(fp);
