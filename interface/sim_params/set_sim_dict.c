...
/*------------------------------------------------------------------------*/ 
 ...
 /*-----------------------------------------------------------------------*/ 
 /*  2)\cp_ggax_typ{B3,becke,pw91x,fila_1x,fil_2x,pbe_x,revpbe_x,rpbe_x,
                    xpbe_x,brx89,brx2k,debug97x,off}*/
        strcpy((*dict)[2].error_mes,
        "B3,becke,pw91x,fila_1x,fila_2x,brx89,brx2k,pbe_x,revpbe_x,rpbe_x,xpbe_x");
  ...
  /* 38)\basis_set_opt{plane_wave,dvr} */
        strcpy((*dict)[38].error_mes,"plane_wave,dvr");
        strcpy((*dict)[38].keyword,"basis_set_opt");
        strcpy((*dict)[38].keyarg,"plane_wave");

  /*-----------------------------------------------------------------------*/
  /* 39)\dvr_grid_dens_def{#} */
        strcpy((*dict)[39].error_mes,"a number > 0");
        strcpy((*dict)[39].keyword,"dvr_grid_dens_def");
        strcpy((*dict)[39].keyarg,"0.75");

  /*-----------------------------------------------------------------------*/
  /* 40)\cp_nl_trunc_opt{#} */
        strcpy((*dict)[40].error_mes,"on,off");
        strcpy((*dict)[40].keyword,"cp_nl_trunc_opt");
        strcpy((*dict)[40].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 41)\cp_wan_min_opt{#} */
        strcpy((*dict)[41].error_mes,"on,off");
        strcpy((*dict)[41].keyword,"cp_wan_min_opt");
        strcpy((*dict)[41].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 42)\cp_wan_opt{#} */
        strcpy((*dict)[42].error_mes,"on,off");
        strcpy((*dict)[42].keyword,"cp_wan_opt");
        strcpy((*dict)[42].keyarg,"off");
  /*-----------------------------------------------------------------------*/
  /* 43)\cp_wan_calc_frq{#} */
        strcpy((*dict)[43].error_mes,"a number >= 0");
        strcpy((*dict)[43].keyword,"cp_wan_calc_frq");
        strcpy((*dict)[43].keyarg,"0");
  /*-----------------------------------------------------------------------*/
  /* 44)\wan_func_typ{#} */
        strcpy((*dict)[44].error_mes,"a number >= 1");
        strcpy((*dict)[44].keyword,"wan_func_typ");
        strcpy((*dict)[44].keyarg,"2");
  /*-----------------------------------------------------------------------*/
  /* 45)\wan_diag_typ{#} */
        strcpy((*dict)[45].error_mes,"a number >= 1");
        strcpy((*dict)[45].keyword,"wan_diag_typ");
        strcpy((*dict)[45].keyarg,"1");
  /*-----------------------------------------------------------------------*/
  /* 46)\cp_dip_calc_frq{#} */
        strcpy((*dict)[46].error_mes,"a number >= 0");
        strcpy((*dict)[46].keyword,"cp_dip_calc_frq");
        strcpy((*dict)[46].keyarg,"0");
  /*-----------------------------------------------------------------------*/
  /* 47)\cp_nloc_wan_opt{#} */
        strcpy((*dict)[47].error_mes,"on,off");
        strcpy((*dict)[47].keyword,"cp_nloc_wan_opt");
        strcpy((*dict)[47].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 48)\cp_kinet_wan_opt{#} */
        strcpy((*dict)[48].error_mes,"on,off");
        strcpy((*dict)[48].keyword,"cp_kinet_wan_opt");
        strcpy((*dict)[48].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 49)\rcut_wan_orb{#} */
        strcpy((*dict)[49].error_mes,"a real number > 0");
        strcpy((*dict)[49].keyword,"rcut_wan_orb");
        strcpy((*dict)[49].keyarg,"1000000.0");

  /*-----------------------------------------------------------------------*/
  /* 50)\rcut_wan_nl{#} */
        strcpy((*dict)[50].error_mes,"a real number > 0");
        strcpy((*dict)[50].keyword,"rcut_wan_nl");
        strcpy((*dict)[50].keyarg,"1000000.0");
  /*-----------------------------------------------------------------------*/
  /* 51)\nmax_wan_orb{#} */
        strcpy((*dict)[51].error_mes,"a number > 0");
        strcpy((*dict)[51].keyword,"nmax_wan_orb");
        strcpy((*dict)[51].keyarg,"1");

  /*-----------------------------------------------------------------------*/
  /* 52)\B3_cutoff{#}   */
        strcpy((*dict)[52].error_mes,"a number > 0");
        strcpy((*dict)[52].keyword,"b3_cutoff");
        strcpy((*dict)[52].keyarg,"3.5");
  /*-----------------------------------------------------------------------*/
  /* 53)\B3_alpha{#}   */
        strcpy((*dict)[53].error_mes,"a number > 0");
        strcpy((*dict)[53].keyword,"b3_alpha");
        strcpy((*dict)[53].keyarg,"1.0");

  /*-----------------------------------------------------------------------*/
  /* 54)\cp_wan_init_opt{#} */
        strcpy((*dict)[54].error_mes,"on,off");
        strcpy((*dict)[54].keyword,"cp_wan_init_opt");
        strcpy((*dict)[54].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 55)\cp_init_min_opt{#} */
        strcpy((*dict)[55].error_mes,"on,off");
        strcpy((*dict)[55].keyword,"cp_init_min_opt");
        strcpy((*dict)[55].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 56)\iwrite_init_wcent{#} */
        strcpy((*dict)[56].error_mes,"on,off");
        strcpy((*dict)[56].keyword,"iwrite_init_wcent");
        strcpy((*dict)[56].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 57)\iwrite_init_worb{#} */
        strcpy((*dict)[57].error_mes,"on,off");
        strcpy((*dict)[57].keyword,"iwrite_init_worb");
        strcpy((*dict)[57].keyarg,"off");

  /*-----------------------------------------------------------------------*/
  /* 58)\iwrite_init_state{#} */
        strcpy((*dict)[58].error_mes,"a number > 0");
        strcpy((*dict)[58].keyword,"iwrite_init_state");
        strcpy((*dict)[58].keyarg,"1");

  /*-----------------------------------------------------------------------*/
  /* 59)\dvr_clus_tmax{#} */
        strcpy((*dict)[59].error_mes,"a number > 0");
        strcpy((*dict)[59].keyword,"dvr_clus_tmax");
        strcpy((*dict)[59].keyarg,"1000");

  /*-----------------------------------------------------------------------*/
  /* 60)\dvr_clus_num_twindow{#}   */
        strcpy((*dict)[60].error_mes,"a number >= 1");
        strcpy((*dict)[60].keyword,"dvr_clus_num_twindow");
        strcpy((*dict)[60].keyarg,"9");

  /*-----------------------------------------------------------------------*/
  /* 61)\dvr_clus_num_tquad{#}   */
        strcpy((*dict)[61].error_mes,"a number >= 1");
        strcpy((*dict)[61].keyword,"dvr_clus_num_tquad");
        strcpy((*dict)[61].keyarg,"12");

  /*-----------------------------------------------------------------------*/
  /* 62)\dvr_clus_grid_dens{#} */
        strcpy((*dict)[62].error_mes,"a number >= 0");
        strcpy((*dict)[62].keyword,"dvr_clus_grid_dens");
        strcpy((*dict)[62].keyarg,"0.2");
...
"minimize,md,pimd,cp,cp_pimd,cp_wave,cp_wave_pimd,cp_min,cp_wave_min,cp_wave_min_pimd");
  ...
  /*-----------------------------------------------------------------------*/
  /*  27)\num_threads_mkl{#} */
        strcpy((*dict)[28].error_mes,"a integer number > 0");
        strcpy((*dict)[28].keyword,"num_threads_mkl");
        strcpy((*dict)[28].keyarg,"1");
...
/*------------------------------------------------------------------------*/ 
  ...
  /* 34)\wan_min_tol{#} */
        strcpy((*dict)[34].error_mes,"a number > 0 ");
        strcpy((*dict)[34].keyword,"wan_min_tol");
        strcpy((*dict)[34].keyarg,"0.005");
...
/*------------------------------------------------------------------------*/ 
  ...
  /*  30)\cp_wan_cent_out_file */
        ...
        strcpy((*dict)[30].keyword,"cp_wan_cent_out_file");
        strcpy((*dict)[30].keyarg,"sim_cp_wcent.out");
  /*-----------------------------------------------------------------------*/
  /*  31)\cp_wan_orb_out_file */
         strcpy((*dict)[31].error_mes,"none");
         strcpy((*dict)[31].keyword,"cp_wan_orb_out_file");
         strcpy((*dict)[31].keyarg,"sim_cp_worb.out");

  /*-----------------------------------------------------------------------*/
  /*  32)\cp_wan_cent_in_file */
        strcpy((*dict)[32].error_mes,"none");
        strcpy((*dict)[32].keyword,"cp_wan_cent_in_file");
        strcpy((*dict)[32].keyarg,"sim_cp_wcent.in");

  /*-----------------------------------------------------------------------*/
  /*  33)\cp_dip_file */
         strcpy((*dict)[33].error_mes,"none");
         strcpy((*dict)[33].keyword,"cp_dip_file");
         strcpy((*dict)[33].keyarg,"sim_cp_dip.out");
...
void set_sim_dict_dafed (int *num_dict, DICT_WORD *dict[]) {
  ...
  *dict = (DICT_WORD *)cmalloc(*num_dict*sizeof(DICT_WORD))-1;
  ...
  /* set dafed keywords */
   strcpy((*dict)[1].error_mes,"number of CVs");
   strcpy((*dict)[1].keyword,"num_cv");
   strcpy((*dict)[1].keyarg,"0");

   strcpy((*dict)[2].error_mes,"S-particle(s) Trajectory");
   strcpy((*dict)[2].keyword,"dafed_traj_name");
   strcpy((*dict)[2].keyarg,"dafed.out");

   strcpy((*dict)[3].error_mes,"d-AFED GGMT Tau");
   strcpy((*dict)[3].keyword,"s_ggmt_tau");
   strcpy((*dict)[3].keyarg,"20");

   strcpy((*dict)[4].error_mes,"d-AFED Data Frequency");
   strcpy((*dict)[4].keyword,"dafed_data_freq");
   strcpy((*dict)[4].keyarg,"5");

   strcpy((*dict)[5].error_mes,"biasing potential off-on");
   strcpy((*dict)[5].keyword,"bias_pot_on");
   strcpy((*dict)[5].keyarg,"off");

   strcpy((*dict)[6].error_mes,"biasing potential read in");
   strcpy((*dict)[6].keyword,"read_bias_pot");
   strcpy((*dict)[6].keyarg,"off");

   strcpy((*dict)[7].error_mes,"biasing potential update freq(#of steps)");
   strcpy((*dict)[7].keyword,"bias_update_freq");
   strcpy((*dict)[7].keyarg,"100");

   strcpy((*dict)[8].error_mes,"biasing potential print freq(# of updating times)");
   strcpy((*dict)[8].keyword,"bias_write_freq");
   strcpy((*dict)[8].keyarg,"10");

   strcpy((*dict)[9].error_mes,"sigma of gaussians");
   strcpy((*dict)[9].keyword,"gau_sigma");
   strcpy((*dict)[9].keyarg,"1.0");

   strcpy((*dict)[10].error_mes,"coeffecient of gaussians");
   strcpy((*dict)[10].keyword,"gau_coef");
   strcpy((*dict)[10].keyarg,"0.0");

   strcpy((*dict)[11].error_mes,"d-AFED GGMT Nr");
   strcpy((*dict)[11].keyword,"s_ggmt_nr");
   strcpy((*dict)[11].keyarg,"1");

   strcpy((*dict)[12].error_mes,"d-AFED r-RESPA steps (an whole number)");
   strcpy((*dict)[12].keyword,"respa_dafed");
   strcpy((*dict)[12].keyarg,"1");
