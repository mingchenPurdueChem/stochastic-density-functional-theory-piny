...
/*========================================================================*/
  ...
  /* 38)\basis_set_opt{plane_wave,dvr}  */
       ifound = 0;
       if(strcasecmp(dict[38].keyarg,"plane_wave")==0)    {
          cp->cpcoeffs_info.iopt_cp_pw  = 1; ifound++;
          cp->cpcoeffs_info.iopt_cp_dvr = 0; }
       if(strcasecmp(dict[38].keyarg,"dvr")==0)    {
          cp->cpcoeffs_info.iopt_cp_dvr = 1; ifound++;
          cp->cpcoeffs_info.iopt_cp_pw  = 0; }
       index=38;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 39)\dvr_grid_dens_def{#} */
       sscanf(dict[39].keyarg,"%lg",&(cp_parse->cp_dvrdens_def));
       index=39;
       if(cp_parse->cp_dvrdens_def <= 0.0)
         keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 40)\cp_nl_trunc_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[40].keyarg,"on")==0)    {
          cp->cpopts.cp_nl_trunc_opt = 1; ifound++;}
       if(strcasecmp(dict[40].keyarg,"off")==0)    {
          cp->cpopts.cp_nl_trunc_opt = 0; ifound++;}
       index=40;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
  ...
  /* 41)\cp_wan_min_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[41].keyarg,"on")==0)    {
          cp->cpopts.cp_wan_min_opt = 1; ifound++;}
       if(strcasecmp(dict[41].keyarg,"off")==0)    {
          cp->cpopts.cp_wan_min_opt = 0; ifound++;}
       index=41;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
  ...
  /* 42)\cp_wan_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[42].keyarg,"on")==0)    {
          cp->cpopts.cp_wan_opt = 1; ifound++;}
       if(strcasecmp(dict[42].keyarg,"off")==0)    {
          cp->cpopts.cp_wan_opt = 0; ifound++;}
       index=42;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
  ...
  /*-----------------------------------------------------------------------*/
  /* 43)\cp_wan_calc_frq{#} */
       sscanf(dict[43].keyarg,"%lg",&real_key_arg);
       cp->cp_wannier.cp_wan_calc_frq =  (int)(real_key_arg);
       index=43;
       if((cp->cp_wannier.cp_wan_calc_frq  <0 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 44)\wan_func_typ{#} */
       sscanf(dict[44].keyarg,"%lg",&real_key_arg);
       cp->cp_wannier.wan_func_typ =  (int)(real_key_arg);
       index=44;
       if((cp->cp_wannier.wan_func_typ  <0 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);
       if(cp->cp_wannier.wan_func_typ  > 3){
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@\n");
         printf(" Unrecognizable spread functional for Wannier\n");
         printf(" orbitals. Choose n= 1,2 or 3 for cp_wan_func_typ{n}\n");
         printf(" where each number is reserved for the following  \n");
         printf(" functionals. \n");
         printf("   1) Resta functional                           \n");
         printf("   2) Silvestrelli functional                   \n");
         printf("   3) Berghold functional                        \n");
         printf(" If you are not sure what to choose, use default.\n");
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@\n");
         fflush(stdout);
         Finalize(); exit(1);
       }
       if(cp->cp_wannier.wan_func_typ  != 2){
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
         printf("Spread functionals other than Silvestrelli type \n");
         printf("have not been tested extensively.\n");
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
       }
  /*-----------------------------------------------------------------------*/
  /* 45)\wan_diag_typ{#} */
       sscanf(dict[45].keyarg,"%lg",&real_key_arg);
       cp->cp_wannier.wan_diag_typ =  (int)(real_key_arg);
       index=45;
       if((cp->cp_wannier.wan_diag_typ  <0 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

       if(cp->cp_wannier.wan_func_typ  > 3){
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@\n");
         printf(" Unrecognizable diagonalization options for Wannier\n");
         printf(" orbitals. Choose n= 1,2 or 3 for cp_wan_diag_typ{n}\n");
         printf(" where each number is reserved for the following  \n");
         printf(" diagonalization methods. \n");
         printf("   1) QR algorithm (slow, but robust)            \n");
         printf("   2) divide and conquer (fast)                  \n");
         printf("   3) Fastest and relatively robust.             \n");
         printf(" If you are not sure what to choose, use default.\n");
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@\n");
         fflush(stdout);
         Finalize(); exit(1);
       }
       if(cp->cp_wannier.wan_func_typ  != 1){
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
         printf("Diagonalization methods other than QR algorithm\n");
         printf("have not been tested extensively.\n");
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
       }
  ...
  /* 46)\cp_dip_calc_frq{#} */
       ...
       cp->cpcoeffs_info.cp_dip_calc_frq =  (int)(real_key_arg);
       index=46;
  ...
  /* 47)\cp_nloc_wan_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[47].keyarg,"on")==0)    {
          cp->cpopts.cp_nloc_wan_opt = 1; ifound++;}
       if(strcasecmp(dict[47].keyarg,"off")==0)    {
          cp->cpopts.cp_nloc_wan_opt = 0; ifound++;}
       index=47;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
       ...
       if(cp->cpopts.cp_nloc_wan_opt == 1){
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
         printf("cp_nloc_wan_opt{} option is in development stage. \n");
         printf("There is no gaurantee that this would work!!!\n");
         printf("$$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$$\n");
       }
  ...
  /* 48)\cp_kinet_wan_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[48].keyarg,"on")==0)    {
          cp->cpopts.cp_kinet_wan_opt = 1; ifound++;}
       if(strcasecmp(dict[48].keyarg,"off")==0)    {
          cp->cpopts.cp_kinet_wan_opt = 0; ifound++;}
       index=48;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

       if(cp->cpopts.cp_kinet_wan_opt == 1){
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
         printf("cp_kinet_wan_opt{} option is reserved for future development.\n");
         printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
         fflush(stdout);
         exit(1);
       }
  ...
  /*-----------------------------------------------------------------------*/
  /* 49)\rcut_wan_orb{#} */
       sscanf(dict[49].keyarg,"%lg",&real_key_arg);
       index=49;
       cp->cp_wannier.rcut_wan_orb = real_key_arg;
       if(cp->cp_wannier.rcut_wan_orb <= 0.0)
         keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 50)\rcut_wan_nl{#} */
       sscanf(dict[50].keyarg,"%lg",&real_key_arg);
       index=50;
       cp->cp_wannier.rcut_wan_nl = real_key_arg;
       if(cp->cp_wannier.rcut_wan_nl <= 0.0)
         keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 51)\nmax_wan_orb{#} */
       sscanf(dict[51].keyarg,"%lg",&real_key_arg);
       cp->cp_wannier.nmax_wan_orb =  (int)(real_key_arg); 
       index=51;
       if((cp->cp_wannier.nmax_wan_orb < 1 )) 
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

       /*
       cp->cpscr.cpscr_nonloc.nmax_wan_orb =  (int)(real_key_arg);
       index=51;
       if((cp->cpscr.cpscr_nonloc.nmax_wan_orb < 1 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);
       */
  ...
  /*-----------------------------------------------------------------------*/
  /* 52)\b3_cutoff{#}   */
       sscanf(dict[52].keyarg,"%lg",&real_key_arg);
       cp->pseudo.b3_cut = (real_key_arg);
       index=52;
       if(cp->pseudo.b3_cut<0){
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 53)\b3_alpha{#}   */
       sscanf(dict[53].keyarg,"%lg",&real_key_arg);
       cp->pseudo.b3_alp = (real_key_arg);
       index=53;
       if(cp->pseudo.b3_alp<0.0){
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 54)\cp_wan_init_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[54].keyarg,"on")==0)    {
          cp->cpopts.cp_wan_init_opt = 1; ifound++;}
       if(strcasecmp(dict[54].keyarg,"off")==0)    {
          cp->cpopts.cp_wan_init_opt = 0; ifound++;}
       index=54;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
  ...
  /* 55)\cp_init_min_opt{#}   */
       ifound = 0;
       if(strcasecmp(dict[55].keyarg,"on")==0)    {
          cp->cpopts.cp_init_min_opt = 1; ifound++;}
       if(strcasecmp(dict[55].keyarg,"off")==0)    {
          cp->cpopts.cp_init_min_opt = 0; ifound++;}
       index=55;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 56)\iwrite_init_wcent{#}   */
       ifound = 0;
       if(strcasecmp(dict[56].keyarg,"on")==0)    {
          cp->cpopts.iwrite_init_wcent = 1; ifound++;}
       if(strcasecmp(dict[56].keyarg,"off")==0)    {
          cp->cpopts.iwrite_init_wcent = 0; ifound++;}
       index=56;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}
  ...
  /* 57)\iwrite_init_worb{#}   */
       ifound = 0;
       if(strcasecmp(dict[57].keyarg,"on")==0)    {
          cp->cpopts.iwrite_init_worb = 1; ifound++;}
       if(strcasecmp(dict[57].keyarg,"off")==0)    {
          cp->cpopts.iwrite_init_worb = 0; ifound++;}
       index=57;
       if(ifound != 1){
          keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 58)\iwrite_init_state{#} */
       sscanf(dict[58].keyarg,"%lg",&real_key_arg);
       cp->cpopts.iwrite_init_state =  (int)(real_key_arg);
       index=58;
       if((cp->cpopts.iwrite_init_state < 1 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 59)\dvr_clus_tmax{#}{#}   */
       sscanf(dict[59].keyarg,"%lg",&real_key_arg);
       cp->cp_dvr_clus.tmax = (real_key_arg);
       index=59;
       if(cp->cp_dvr_clus.tmax<0.0){
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);}

  /*-----------------------------------------------------------------------*/
  /* 60)\dvr_clus_num_twindow{#} */
       sscanf(dict[60].keyarg,"%lg",&real_key_arg);
       cp->cp_dvr_clus.num_twindow =  (int)(real_key_arg);
       index=60;
       if((cp->cp_dvr_clus.num_twindow <1 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 61)\dvr_clus_num_tquad{#} */
       sscanf(dict[61].keyarg,"%lg",&real_key_arg);
       cp->cp_dvr_clus.num_tquad =  (int)(real_key_arg);
       index=61;
       if((cp->cp_dvr_clus.num_tquad <1 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 62)\dvr_clus_grid_dens{#} */
       sscanf(dict[62].keyarg,"%lg",&real_key_arg);
       cp->cp_dvr_clus.grid_dens =  (real_key_arg);
       index=62;
       if((cp->cp_dvr_clus.grid_dens <0 ))
        keyarg_barf(dict,filename_parse->input_name,fun_key,index);

  /*-----------------------------------------------------------------------*/
  /* 63)\dvr_clus_rmax{#} */
       sscanf(dict[63].keyarg,"%lg",&real_key_arg);
       cp->cp_dvr_clus.rmax =  (real_key_arg);
       index=63;
...
/*        class->clatoms_info.hess_calc=0; */
  ...
  /* 33)\auto_rescale_cp_vel_tol{ # } */
   ...
   /*-----------------------------------------------------------------------*/
   /* 34)\wan_min_tol{#} */
       sscanf(dict[34].keyarg,"%lg",&(general_data->minopts.tol_wan_coef));
       index=34;
       if(general_data->minopts.tol_wan_coef<=0)
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);
...
/*========================================================================*/
  ...
  /*-----------------------------------------------------------------------*/
  /* 30)\cp_wan_cent_out_file */
       sscanf(dict[30].keyarg,"%s",general_data->filenames.wcoutname);
  /*-----------------------------------------------------------------------*/
  /* 31)\cp_wan_orb_out_file */
       sscanf(dict[31].keyarg,"%s",general_data->filenames.worbname);
  /*-----------------------------------------------------------------------*/
  /* 32)\cp_wan_cent_in_file */
        sscanf(dict[32].keyarg,"%s",general_data->filenames.wcinname);
  /*-----------------------------------------------------------------------*/
  /* 33)\cp_dip_file */
        sscanf(dict[33].keyarg,"%s",general_data->filenames.dipname);
...
/* IX) Wannier checks                                                         */
  ...
  if(cp->cp_wannier.cp_wan_calc_frq > 0){
   general_data->filenames.iwrite_wan_cent=cp->cp_wannier.cp_wan_calc_frq;
  }else{
   general_data->filenames.iwrite_wan_cent=general_data->timeinfo.ntime+1;
  } /* endif */

  if(cp->cpcoeffs_info.cp_dip_calc_frq > 0){
    general_data->filenames.iwrite_dipole = cp->cpcoeffs_info.cp_dip_calc_frq;
  }else{
    general_data->filenames.iwrite_dipole = general_data->timeinfo.ntime+1;
  }

  if((cp->cpopts.cp_wan_opt==1 || cp->cpopts.cp_wan_min_opt==1) ||
      cp->cpopts.cp_wan_init_opt == 1){
   printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
   printf("You have chosen to calculate the wannier orbitals. \n");
   printf("Be careful, your simulation box MUST BE orthorombic.\n");
   printf("Non-orthorhombic case has not been implemented.\n");
   printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n\n");
   fflush(stdout);
  } /* endif */

  if((cp->cpopts.cp_wan_opt==1 || cp->cpopts.cp_wan_min_opt==1) &&
      cp->cpopts.cp_wan_init_opt == 0){

   printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
   printf("You chose to run Wannier cp dynamics, but the initial\n");
   printf("rotation onto Wannier gauge is not selected. Make it\n");
   printf("sure that orbitals in the restart file are on the\n" );
   printf("Wannier gauge. \n");
   printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n\n");
   fflush(stdout);
  ...
  if((cp->cpopts.cp_wan_opt==1 || cp->cpopts.cp_wan_min_opt==1)&&
      (cp->cpopts.cp_lsda==1)){
    printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
    printf("The spin-polarized DFT is not yet available for\n");
    printf(" Wannier orbitals\n");
    printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@\n");
    fflush(stdout);
    Finalize();
    exit(1);
  ...
  if(cp->cpopts.cp_nloc_wan_opt==1 && cp->cpopts.cp_wan_opt==0
     && cp->cpopts.cp_wan_min_opt==0){
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
    printf(" You choose cp_nloc_wan_opt on, but not cp_wan_opt\n");
    printf(" or cp_wan_min_opt. You have to run Wannier dynamics\n");
    printf(" if you want to truncate orbitals in nonlocal\n");
    printf(" energy/force calculation. \n");
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@\n");
    fflush(stdout);
    Finalize(); exit(1);
  }
  ...
  if((cp->cpopts.cp_wan_opt==1 || cp->cpopts.cp_wan_min_opt==1)
     && cp->cp_wannier.cp_wan_calc_frq < 1){
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@@\n");
    printf(" You choose to run Wannier dynamics, but you also set \n");
    printf(" cp_wan_calc_frq < 1. This is not compatible each other.\n");
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@@\n");
    fflush(stdout);
    Finalize(); exit(1);
  }
  ...
  if(cp->cpopts.cp_wan_opt==1 && cp->cpopts.cp_wan_min_opt==1){
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@@\n");
    printf(" You set both cp_wan_opt and cp_wan_min_opt to be 1,\n");
    printf(" but this is not allowed. You have to choose either\n");
    printf(" full minimization (cp_wan_min_opt) or\n");
    printf(" on-the-fly minimization (cp_wan_opt). \n");
    printf("@@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@@@\n");
    fflush(stdout);
    Finalize(); exit(1);
  }
...
/* IX) DVR/PW checks                                                         */
   ...
   if(cp->cpcoeffs_info.iopt_cp_pw  == 1) {
     if(cp->cpopts.cp_nl_trunc_opt == 1){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("You choose spherical truncation option for non-local     \n");
       printf("pseudopotential. This is not allowed for plane wave    \n");
       printf("calculations \n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
   ...
   if(cp->cpcoeffs_info.iopt_cp_dvr  == 1){
     ...
     if(iperd !=0 && iperd !=3) {
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Surface (2D) and wire (1D) boundary conditions have not\n");
       printf("been implemented for DVR basis\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }
     ...
     if(cp->cpopts.cp_lsda == 1){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Spin-polarized calculation is implemented for DVR, but \n");
       printf("it has not been tested and it probably does not work!!\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }
     ...
     printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
     printf("Only LDA and BLYP exchange correlation functionals\n");
     printf("are implemented for DVR calculations for now. \n");
     printf("Make it sure you have chosen correct functional. \n");
     printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");

     if(cp->cpopts.cp_norb != 0){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("cp_norb is currently being under development for DVR basis.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }
     ...
     if(cp->cpopts.cp_ptens_calc ==1){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Pressure tensor calculation has not been implemented \n");
       printf("for DVR basis.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }

     if(general_data->minopts.cp_min_cg  != 1 && (general_data->simopts.cp_wave_min == 1
        || general_data->simopts.cp_min == 1)){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Only the conjugate-gradient minimization scheme has been\n");
       printf("implemented for energy minimization with DVR basis.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }

     if(cp->cpopts.cp_para_opt == 0){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("There is no need to use hybrid option for DVR calculation.\n");
       printf("Use full_g option.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }

     if(cp->cpopts.cp_dual_grid_opt != 0){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Dual grid option has not been implemented for DVR calculation.\n");
       printf("set cp_dual_grid_opt to be zero.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }
     ...
     if(cp->cpopts.cp_isok_opt == 1){
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       printf("Isokinetic option is currently being under development \n");
       printf("for DVR basis set.\n");
       printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
       fflush(stdout);
       exit(1);
     }
...
void set_sim_params_dafed(CLASS *class, GENERAL_DATA *general_data, DICT_WORD *dict)
/*=======================================================================*/
/*             Begin routine                                              */
{/*begin routine */
/*=======================================================================*/
/*             Local variable declarations                                */
  ...
  //clatoms_info->dinfo = (DAFED_INFO*)cmalloc(sizeof(DAFED_INFO));
  //dinfo = &(clatoms_info->dinfo);

  /*-----------------------------------------------------------------------*/
  /*  1)\num_cv{#} */
  sscanf(dict[1].keyarg,"%lg",&rka);
  dinfo->n_cv = (int)(rka);
  n_cv = dinfo->n_cv;
  /*-----------------------------------------------------------------------*/
  /*  2)\dafed_traj_name{#} */
  dinfo->traj_fn = (char*)cmalloc(MAXWORD*sizeof(char));
  strcpy(dinfo->traj_fn,dict[2].keyarg);
  /*-----------------------------------------------------------------------*/
  /*  3)\s_ggmt_tau{#} */
   sscanf(dict[3].keyarg,"%lg",&rka);
   dinfo->ggmt_tau = (double)(rka);
  /*-----------------------------------------------------------------------*/
  /*  4)\dafed_data_freq{#} */
   sscanf(dict[4].keyarg,"%lg",&rka);
   dinfo->write_freq = (int)(rka);
  /*-----------------------------------------------------------------------*/
  /*  5)\bias_pot_on{#} */
  if(strcasecmp(dict[5].keyarg,"off")==0)dinfo->bias_on = 0;
  if(strcasecmp(dict[5].keyarg,"on")==0)dinfo->bias_on = 1;
  /*-----------------------------------------------------------------------*/
  /*  6)\read_bias_pot{#} */
  if(strcasecmp(dict[6].keyarg,"off")==0)dinfo->bias_readin = 0;
  if(strcasecmp(dict[6].keyarg,"on")==0)dinfo->bias_readin = 1;
  /*-----------------------------------------------------------------------*/
  /*  7)\bias_update_freq{#} */
  sscanf(dict[7].keyarg,"%lg",&rka);
  dinfo->steps_bias = (int)(rka);
  /*-----------------------------------------------------------------------*/
  /*  8)\bias_write_freq{#} */
  sscanf(dict[8].keyarg,"%lg",&rka);
  dinfo->bias_write_freq = (int)(rka);
  /*-----------------------------------------------------------------------*/
  /*  9)\gau_sigma{#} */
  sscanf(dict[9].keyarg,"%lg",&rka);
  dinfo->sigma = (double)(rka);
  /*-----------------------------------------------------------------------*/
  /*  10)\gau_coef{#} */
  ...
  if(n_cv>0){
    dinfo->dafed_on = 1;
    clatoms_info->dafed = (DAFED*)cmalloc(n_cv*sizeof(DAFED));
  }
...
/*             Local variable declarations                                */
  ...
  if(strcasecmp(dict[14].keyarg,"diis")==0)stodftInfo->densityMixFlag = 2;
  if(strcasecmp(dict[14].keyarg,"mix-diis")==0)stodftInfo->densityMixFlag = 3;
  /*-----------------------------------------------------------------------*/
  /*  15)\mix_steps{#} */
