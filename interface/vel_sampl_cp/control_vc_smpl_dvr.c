...
void control_vc_smpl_dvr(GENERAL_DATA *general_data,CP *cp)
...
/* II) Checks */
  ...
  if(np_states>1){
    ...
    for(ip=1;ip<=pi_beads;ip++){
     if((cp->cpcoeffs_pos_dvr[ip].ivcoef_form_up
        +cp->cpcoeffs_pos_dvr[ip].icoef_form_up)!=2){
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
     ...
     if(cp_lsda==1){
      if((cp->cpcoeffs_pos_dvr[ip].ivcoef_form_dn 
         +cp->cpcoeffs_pos_dvr[ip].icoef_form_dn)!=2){ 
       printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
  ...
  if(cp_norb>0){
   ...
   for(ip=1;ip<=pi_beads;ip++){
    if(cp->cpcoeffs_pos_dvr[ip].icoef_orth_up!=0){
      printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
    ...
    if(cp_lsda==1){
     if(cp->cpcoeffs_pos_dvr[ip].icoef_orth_dn!=0){
       printf("@@@@@@@@@@@@@@@@@@@@_ERROR_@@@@@@@@@@@@@@@@@@@@\n");
...
/* III) Sample velocities                                             */
  ...
  if(myid==0){
    printf("$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$\n");
    printf("           IMPLEMENT SAMPL_VC_DVR  \n");
    printf(" Initial cp coefficient velocities are zero \n");
    printf("$$$$$$$$$$$$$$$$$$$_WARNING_$$$$$$$$$$$$$$$$$$$\n");
  }
  /*
  sampl_vc_dvr(&(cp->cpopts),
               &(cp->cpcoeffs_info),(cp->cpcoeffs_pos_dvr),
	       &(general_data->statepoint),&(cp->vel_samp_cp.iseed),
	       &(cp->vel_samp_cp.iseed2),&(cp->vel_samp_cp.qseed), 
               &(cp->communicate),&(cp->cp_comm_state_pkg_up_dvr),
               &(cp->cp_comm_state_pkg_dn_dvr));
  */
