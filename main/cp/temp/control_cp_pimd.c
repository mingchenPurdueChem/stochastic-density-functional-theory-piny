...
/* II) Loop over the specified number of time steps */
  ...
  /*  4)Rotate the orbitals:                                             */
    ...
    cp_rotate_control(cp,itime,&(general_data->stat_avg.count_diag_srot));
  ...
  /*---------------------------------------------------------------------*/
  /*  12) Analysis Routine                                               */
    analysis_cp_pimd(class,bonded,general_data,cp); 
 ...
 if(myid_state==0){
  if(general_data->simopts.cp==1||general_data->simopts.cp_pimd==1){
    if((general_data->ensopts.nvt)==1){
      if(myid_bead==0){
        iflag_mass = 1;
        init_NHC(&(class->clatoms_info),&(class->clatoms_pos[1]),
                     &(class->therm_info_class),&(class->therm_class),
                    &(class->int_scr),iflag_mass);
        iflag_mass = 2;
        for(ip=2;ip<=pi_beads_proc;ip++){
             init_NHC(&(class->clatoms_info),&(class->clatoms_pos[ip]),
    ...
    if((general_data->ensopts.npt_i)==1){
      if(myid_bead==0){
        init_NHCPI(&(class->clatoms_info),&(class->clatoms_pos[1]),
                   &(class->therm_info_class),&(class->therm_class),
                   &(general_data->baro),
                   &(class->int_scr));
      }/*endif*/
    ...
    if((general_data->ensopts.npt_f)==1){
       init_NHCPF_pimd(&(class->clatoms_info),(class->clatoms_pos),
                       &(class->therm_info_class),&(class->therm_class),
                       &(class->therm_info_bead),(class->therm_bead),
                       &(general_data->baro),&(general_data->par_rahman),
                       &(general_data->cell),
                       &(class->int_scr),myid_bead,comm_beads);
       iflag = 2;
       nhc_vol_potkin_pimd(class,general_data,iflag);
    }/*endif*/
...
if(num_proc>1){
  ...
  Reduce(&(general_data->stat_avg.vrecip), &vrecip_temp,1,MPI_DOUBLE,
                   MPI_SUM,0,world);
 if(myid_state==0){
