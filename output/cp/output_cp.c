...
/*======================================================================*/
/* IX) Write to the dipole moment file                                */

  if(myid==0){
    if((general_data->timeinfo.itime)!=0){
      if((general_data->timeinfo.itime %
          general_data->filenames.iwrite_dipole) == 0 ){
        write_dipole_file_cp(general_data,cp);
      }
    }
  }

/*======================================================================*/
/* X) Write to the Wannier center file                                */

  if(myid==0){
    if(cp->cpopts.cp_wan_opt==1 || cp->cpopts.cp_wan_min_opt==1){
      if((general_data->timeinfo.itime)!=0){
        if((general_data->timeinfo.itime %
            general_data->filenames.iwrite_wan_cent) == 0 ){
          write_wan_cent_file_cp (general_data,cp);
        }
      }
    }
  }
...
/*======================================================================*/
/* XI) Readjust form of CP vectors                                    */
...
/* Output stuff */
  ...
  if((general_data->simopts.cp==1)||(general_data->simopts.debug_cp==1)){
    ...
    printf("Initial e-Hartree  energy  %.10g\n",
                            (general_data->stat_avg.cp_ehart)*eu_conv);

    printf("Initial e-ExCorr  energy  %.10g\n",
                             (general_data->stat_avg.cp_exc)*eu_conv);
...
/*     A) Standard                                                   */
   ...
   printf("e-Hartree         = %.10g %.10g\n",general_data->stat_avg.cp_ehart,
                                           (general_data->stat_avg.acp_ehart)/atime);

   printf("e-Xchange Corr.   = %.10g %.10g\n",general_data->stat_avg.cp_exc,
                                      (general_data->stat_avg.acp_exc)
                                            /atime);
...
/* Write the coefficients                                                  */
  ...
  if(iopt_cp_pw)  {write_dump_coef_cp(fp_dnamec,cp,class,ibinary);}
  if(iopt_cp_dvr) {write_dump_coef_cp_dvr(fp_dnamec,cp,class,ibinary);}
...
/* Write the coefficient velocities                                         */
  ...
  if(iopt_cp_pw) {write_dump_vcoef_cp(fp_dnamec,cp,class,general_data,ibinary);}
  if(iopt_cp_dvr){write_dump_vcoef_cp_dvr(fp_dnamec,cp,class,general_data,ibinary);}
...
void write_dipole_file_cp(GENERAL_DATA *general_data,CP *cp)
{
...
/*                         Begin Routine                                 */
/*=======================================================================*/
/*            Comment : everything happens on node 0                     */
...
#include "../typ_defs/typ_mask.h"
  ...
  int i,j;
  FILE *out_dipole;
  static int first_opening=1;

  if(first_opening==1){
    out_dipole = cfopen(general_data->filenames.dipname,"w");
    fprintf(out_dipole,"  DX(Debye)        DY(Debye)       DZ(Debye)       Bead\n\n");
    fclose(out_dipole);
  }

  out_dipole = cfopen(general_data->filenames.dipname,"a");

  for(i=1;i<=tot_beads;i++){
    fprintf(out_dipole,"  %- 18.5e%- 18.5e%- 18.5e%-8d\n",
       cp->electronic_properties.cp_dip[i][1],cp->electronic_properties.cp_dip[i][2],
       cp->electronic_properties.cp_dip[i][3],i);
  }
  fclose(out_dipole);
...
void write_wan_cent_file_cp(GENERAL_DATA *general_data,CP *cp)
{
...
#include "../typ_defs/typ_mask.h"
  ...
  int nstate=cp->cpcoeffs_info.nstate_up;
  int i,j,tot_beads;
  FILE *out_wan_cent;
  static int first_opening=1;
  double *WCF;
  ...
  if(first_opening==1){
    out_wan_cent = cfopen(general_data->filenames.wcoutname,"w");
    fprintf(out_wan_cent,"  WFC X(A)        WFC Y(A)       WFC Z(A)       State\n\n");
    fclose(out_wan_cent);
  }

  out_wan_cent = cfopen(general_data->filenames.wcoutname,"a");

  for (i=1;i<=nstate;i++){
    WCF=cp->electronic_properties.wannier_cent[i];
    fprintf(out_wan_cent,"  %- 18.5e%- 18.5e%- 18.5e%-8d\n",
                             WCF[1], WCF[2], WCF[3],i);
  }
  fprintf(out_wan_cent,"$$\n");
  fclose(out_wan_cent);
