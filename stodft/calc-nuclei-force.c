...
/* VII) Output the energy Term                                          */
  ...
  if(myidState==0){
    ...
    if(smearOpt>0)energyTotElec += stodftInfo->entropy;
...
/*cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*/
/*==========================================================================*/
void calcEnergyForceFilterDiag(CLASS *class,GENERAL_DATA *general_data,CP *cp,BONDED *bonded,
			CPCOEFFS_POS *cpcoeffs_pos,CLATOMS_POS *clatoms_pos)
/*==========================================================================*/
/*         Begin Routine                                                    */
   {/*Begin Routine*/
...
/* VII) Output the energy Term                                          */
  ...
  if(myidState==0){
    ...
    if(smearOpt>0){
      ...
      energyTot += entropy*smearTemperature;
    }
    printf("Electron Total Elec Energy:   %.20lg\n",energyTotElec);
    printf("Atom Energy:		  %.20lg\n",vInter);
    printf("Total Energy:		  %.20lg\n",energyTot);
