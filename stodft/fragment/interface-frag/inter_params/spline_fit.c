...
void vgrimme_bin(int n, double r[], double v[],double dv[], double sw[],
                  double dsw[],double rm, double c6)
...
/*      Local variables       */
  ...
  int i;
  double vnow,dvnow;
  double r2,r6,r7,r8,d,eee,den,fd,dfd;
...
/* Grimme Potential */
  ...
  for(i=1;i<=n;i++){
    ...
    r7    = r6*r[i];
    r8    = r6*r2;
    eee   = exp(-d*(r[i]/rm-1.0));
    den   = 1.0 + eee;
    fd    = 1.0/den;
    dfd   = (d/rm)*fd*eee/den;
    vnow  = -c6*fd/r6;
    dvnow = -6.0*c6*fd/r8 + c6*dfd/r7;
    v[i]  = vnow*sw[i];
    dv[i] = dvnow*sw[i] + vnow*dsw[i];
  }/*endfor*/
