...
/*========================================================================*/
  ...
  /* 2)\cp_ggax_typ{becke,pw91x,debug97x,fila_1x,fila_2x,pbe_x,revpbe_x,rpbe_x,
       ...
       if(strcasecmp(cp->pseudo.vxc_typ,"pw_lda")!=0  &&
	  strcasecmp(cp->pseudo.vxc_typ,"pw_lsda")!=0 &&
          ((strcasecmp(cp->pseudo.ggax_typ,"pbe_x") == 0) ||
	  (strcasecmp(cp->pseudo.ggax_typ,"revpbe_x") == 0) ||
	  (strcasecmp(cp->pseudo.ggax_typ,"rpbe_x") == 0) ||
	  (strcasecmp(cp->pseudo.ggax_typ,"xpbe_x") == 0))) {
        printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
        printf("In order to use the PBE GGA functional, \n");
        printf("You must choose pw_lda or pw_lsda as the cp_vxc_typ\n");
        printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
        fflush(stdout);
        exit(1);
       }/* endif */
  ...
  /* 3)\cp_ggac_typ{lyp,lypm1,pw91c,pbe_c,xpbe_c,tau1_c,off}               */
	...
	keyarg_barf(dict,filename_parse->input_name,fun_key,index);
      ...
      if(strcasecmp(cp->pseudo.vxc_typ,"pw_lda" )!=0  &&
	 strcasecmp(cp->pseudo.vxc_typ,"pw_lsda" )!=0 &&
	 ((strcasecmp(cp->pseudo.ggac_typ,"pbe_c") == 0) || 
	 (strcasecmp(cp->pseudo.ggac_typ,"xpbe_c") == 0))) {
        printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");  
        printf("In order to use the PBE GGA functional, \n");
        printf("You must choose pw_lda or pw_lsda as the cp_vxc_typ\n");
        printf("@@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
  ...
  /*  6)\cp_norb{full_ortho,norm_only,no_constrnt,off} */
    ...
    if(strcasecmp(dict[6].keyarg,"no_constrnt")==0) {cp->cpopts.cp_norb = 3;ifound++;}
