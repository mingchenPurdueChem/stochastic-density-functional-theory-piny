...
void control_vps_params(PSEUDO *pseudo,CELL *cell,
                        ...
                        int cp_ptens_calc, int cp_dual_grid_opt,
...
/* III) Malloc up the vps stuff                                             */ 
   ...
   now_mem = (natm_typ_mall*(sizeof(double)*7 + sizeof(int))*8)*1.e-06;
  *tot_memory += now_mem;
...
void set_vps_params(DICT_WORD vps_dict[],char *filename,char *fun_key,
                    ...
                    double *phi0_0, double *phi0_1, double *phi0_2)
...
/*  2) Assign non-local info                                                */
       ...
       sscanf(vps_dict[12].keyarg,"%lg",&tmp);
       (*nl_alp) = tmp;
       if((*nl_alp)<0){index=12;
                 keyarg_barf(vps_dict,filename,fun_key,index);}

       sscanf(vps_dict[13].keyarg,"%lg",&tmp);
       (*nl_beta) = tmp;
       if((*nl_beta)<0){index=13;
                 keyarg_barf(vps_dict,filename,fun_key,index);}

       (*nl_filter) = -1;
       if(strcasecmp(vps_dict[14].keyarg,"on") == 0){(*nl_filter) = 1;}
       if(strcasecmp(vps_dict[14].keyarg,"off") == 0){(*nl_filter) = 0;}
       if((*nl_filter)==-1){index=14;
                 keyarg_barf(vps_dict,filename,fun_key,index);}
...
/*  6) phi0 info                                           */
       ...
       sscanf(vps_dict[15].keyarg,"%lg",&tmp);
       (*phi0_0)  = tmp;
       sscanf(vps_dict[16].keyarg,"%lg",&tmp);
       (*phi0_1)  = tmp;
       sscanf(vps_dict[17].keyarg,"%lg",&tmp);
       (*phi0_2)  = tmp;

       if((*phi0_0)<0.0){index=15;
                 keyarg_barf(vps_dict,filename,fun_key,index);}
       if((*phi0_1)<0.0){index=16;
                 keyarg_barf(vps_dict,filename,fun_key,index);}
       if((*phi0_2)<0.0){index=17;
                 keyarg_barf(vps_dict,filename,fun_key,index);}
